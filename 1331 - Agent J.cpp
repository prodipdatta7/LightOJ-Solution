...
int main() {
    ...
    while(t--) {
        ...
        double pi = acos(-1.0) ;
        cin >> ar >> br >> cr ;
        double a = ar + br , b = ar + cr , c = br + cr ;
        double angle_a = ((b * b) + (c * c) - (a * a)) / (2.0 * b * c) ;
        double angle_b = ((a * a) + (c * c) - (b * b)) / (2.0 * a * c) ;
        double angle_c = ((b * b) + (a * a) - (c * c)) / (2.0 * b * a) ;
        angle_c = acos(angle_c) ;
        angle_b = acos(angle_b) ; 
        angle_a = acos(angle_a) ;
        double degree_a = (180.0 * angle_a) / pi ;
        double degree_b = (180.0 * angle_b) / pi ;
        double degree_c = (180.0 * angle_c) / pi ;
        double s = (a + b + c) / 2.0 ;
        double triangle_area = sqrt(s * (s - a) * (s - b) * (s - c)) ;
        double arc_area_a = (degree_a / 360.0) * pi * cr * cr ;
        double arc_area_b = (degree_b / 360.0) * pi * br * br ;
        double arc_area_c = (degree_c / 360.0) * pi * ar * ar ;
        double res = triangle_area - (arc_area_a + arc_area_b + arc_area_c) ;
        printf("Case %d: %.10f\n",++tc,res);
    }
    return 0;
}
