...
struct point{
    ...
    bool operator<(const point &p)const{
        if(p.lx == lx and p.ly == ly)return lz < p.lz ;
        else if(lx == p.lx)return ly < p.ly ;
        return lx < p.lx ;
    }
};
...
int main() {
    ...
    while(t--) {
        ...
        point A[n + 1] ;
        for(int i = 1 ; i <= n ; i++){
            int x = Int , y = Int , z = Int , u = Int , v = Int , p = Int ;
            A[i].lx = x ;
            A[i].ly = y ;
            A[i].lz = z ;
            A[i].rx = u ;
        ...
        sort(A + 1 , A + n + 1) ;
        std::vector< int > f(7);
        f[1] = A[1].lx , f[2] = A[1].ly , f[3] = A[1].lz ;
        f[4] = A[1].rx , f[5] = A[1].ry , f[6] = A[1].rz ;
        for(int i = 2 ; i <= n ; i++){
            f[1] = max(f[1] , A[i].lx) ;
            f[2] = max(f[2] , A[i].ly) ;
            f[3] = max(f[3] , A[i].lz) ;
        ...
        //for(int i = 1 ; i <= 6 ; i++)error(f[i])
        ll res = 0 ;
        if(f[1] > f[4] or f[2] > f[5] or f[3] > f[6]){
            res = 0 ;
        }
        else{
            int a = f[4] - f[1] , b = f[5] - f[2] , c = f[6] - f[3] ;
            res = 1LL * a * b * c ;
        }
        printf("Case %d: %lld\n",++tc , res);
    }
