...
int main() {
    ...
    while(t--) {
    	int n = Int , k = Int ;
    	int trail = modpow(1LL * n , 1LL * k , 1000LL) ;
    	double f = 1.0 * k * log10(n) ;
    	double q = f - floor(f + eps) ;
    	double B = pow(10 , q) ;
    	for(int i = 1 ; i <= 2 ; i++){
    		B *= 10 ;
    	}
    	printf("Case %d: %d ",++tc , (int)B);
    	if(trail < 10)printf("00%d\n",trail);
    	else if(trail < 100)printf("0%d\n",trail);
    	else printf("%d\n",trail);
    }
    return 0;
}
