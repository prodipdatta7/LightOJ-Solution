...
using namespace std;
const int N =1e5+5;
const long long M = (long long)1e8+7;
#define     ll      long long
...
int main() {
    ...
    int t = Int, tc = 0 ;
    while(t--) {
        ll a = Long, b = Long , ans = 0 ;
        ll n = upper_bound(all(pos),b) - lower_bound(all(pos),a);
        if(n > 0 ){
        ll m = 2 * n ;
        ans = res[m] * ModInv((res[m-n]*res[n]) % M, M);
        ans %= M ;
        ans -= res[m] * ModInv( (res[n+1]*res[m-n-1]) % M, M);
        ans %= M ;
        if(ans < 0)
            ans += M ;
        }
