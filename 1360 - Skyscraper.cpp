/**
     ...
     *
     *  created : Sunday 18-October, 2020  11:21:53 PM
**/
...
struct Orders {
	int start, end, cost ; 
	Orders(int s = 0, int e = 0, int c = 0) : start(s), end(e), cost(c) {}
	bool operator<(const Orders& p)const {
		return make_pair(end, start) < make_pair(p.end, p.start)  ;
	}
} ; 
...
struct FenwickTree{
    ...
    FenwickTree(int n):n(n){T.resize(n + 1, 0);}
    
    inline void update(int i, int v){while(i <= n)cmax(T[i], v), i += i & -i ;}
    
    inline int query(int i){return !i ? 0 : max(T[i], query(i - (i & -i))) ;}
};
...
int main() {
    ...
    while (test--) {
    	int n ; Int(n) ; 
    	vector < Orders > container(n) ; 
    	for(int i = 0 ; i < n ; ++i){
    		Int(container[i].start, container[i].end, container[i].cost) ; 
    		container[i].end += container[i].start ; 
    		container[i].start++ ; 
    	}
    	sort(all(container)) ; 
    	FenwickTree helper(N) ; 
    	for(int i = 0 ; i < n ; ++i){
    		int unoccupied = container[i].start - 1 ; 
    		int optimalProfit = max(helper.query(unoccupied) + container[i].cost, helper.query(container[i].end)) ; 
    		helper.update(container[i].end, optimalProfit) ; 
    	}
    	int FinalProfit = helper.query(N) ; 
    	printf("Case %d: %d\n",++tc, FinalProfit);
    }
